...
WCL 신화 냉법 랭킹(최신 파티션)을 받아 V2 combatantInfo 로 장신구를 붙이고,
"평균 딜"보다 "99 파스 확률을 올리는 세팅" 관점의 JSON 리포트를 만든다.

BM 버전(analyze_bm_trinket_recommendations.py)과 달리 장신구 ID 를 하드코딩하지
않는다 — 냉법 상위권 장비(v2 player_fight 캐시)에 실제로 등장한 장신구만 집계.
...
OUT_JSON = DATA / "frost_trinket_recommendations.json"
...
def load_hero_nodes() -> dict[str, set[int]]:
    """talent_trees.json 에 Mage/Frost 영웅트리가 있으면 {이름: 노드셋} 반환.

    현재 talent_trees.json 에 Mage/Frost 가 없으면 {} — 영웅 구분 없이 전체 집계.
    """
    try:
        tt = json.loads(TALENT_TREES.read_text(encoding="utf-8"))
    except Exception:
        return {}
    hero = (tt.get("Mage/Frost") or {}).get("hero") or {}
...
def main() -> None:
    ...
    output = {
        "_meta": {
            ...
            "notes": [
                ...
                "장신구 목록은 하드코딩 없이 냉법 상위권 장비에 실제로 나온 것만 집계합니다.",
    ...
    try:
        ...
        record(
            action="analyze_frost_trinket_recommendations",
