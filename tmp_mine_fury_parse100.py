"""분노 전사(Fury) '올100' 갭 분석 — rank 1~5(사실상 100점) vs 21~100 실측 비교.

패턴은 tmp_mine_mm_parse100.py 재사용. events 원본 스캔 대신 스크래치패드의
fury_cd_events.json(쿨기 분석 때 v2_cache_events.json 1.4GB에서 추출한 casts+buffs
캐시, 2026-07-11 01:34 생성 — events 캐시 최종 갱신 07-10 23:52 이후)을 재사용.
★오늘자 rankings CSV에 있는 킬만 대상 (CSV report_id/fight_id로 필터)★
★영웅특성(산왕/학살자)이 갈리므로 실행 지표는 영웅트리별로도 분리★
★주의: events 백필이 보스당 밴드별 층화표본(1~5위 5명 / 6~20위 15명 / 21~100위 13명)★

1) casts 기반: 분당 광란(주 스펜더), 분당 피의갈증(피범벅 포함), 분당 분노의강타(분쇄의타격 포함),
   분당 마무리일격, 분당 총 시전, 다운타임(3초+ 무시전 구간 합), 무모한희생 분당 사용
...
출력: data/fury_parse100_gap.json
"""
...
CSV_CLASS, CSV_SPEC = "Warrior", "Fury"
CACHE_NAME = "fury_cd_events.json"
OUT_NAME = "fury_parse100_gap.json"
HEROES = ("산왕", "학살자")

RAMPAGE = 184367                 # 광란 (주 스펜더)
BLOODTHIRST = {23881, 335096}    # 피의 갈증 + 피범벅(강화판)
RAGINGBLOW = {85288, 335097}     # 분노의 강타 + 분쇄의 타격(강화판)
EXECUTE = {280735}               # 마무리 일격
RECK = 1719                      # 무모한 희생 (주기 쿨기)
FILLER = {190411, 6343, 435222}  # 소용돌이/천둥벼락/우레 작렬 (광역 태깅 필러)
...
def fight_metrics(e, t0, t1):
    ...
    return {
        "dur": dur,
        "spender_pm": ids.get(RAMPAGE, 0) / (dur/60),
        "bt_pm": sum(ids.get(x, 0) for x in BLOODTHIRST) / (dur/60),
        "rb_pm": sum(ids.get(x, 0) for x in RAGINGBLOW) / (dur/60),
        "exec_pm": sum(ids.get(x, 0) for x in EXECUTE) / (dur/60),
        "cd_pm": ids.get(RECK, 0) / (dur/60),
...
def summarize(ms):
    ...
    return {
        "n": len(ms),
        "rampage_per_min_med": med("spender_pm"),
        "bloodthirst_per_min_med": med("bt_pm"),
        "ragingblow_per_min_med": med("rb_pm"),
        "execute_per_min_med": med("exec_pm"),
        "recklessness_per_min_med": med("cd_pm"),
        ...
        "aoe_filler_pct_med": round(pct(filler, .5), 1) if filler else None,
    }
...
def main():
    ...
    out = {"meta": {
        ...
        "spell_ids": {"rampage": RAMPAGE, "bloodthirst_incl_bloodbath": sorted(BLOODTHIRST),
                      "ragingblow_incl_crushing": sorted(RAGINGBLOW), "execute": sorted(EXECUTE),
                      "recklessness": RECK, "aoe_filler": sorted(FILLER)},
    ...
    for boss, lst in sorted(bosses.items(), key=lambda x: -len(x[1])):
        ...
        for b in ("rank_1_5", "rank_6_20", "rank_21_100"):
            s = d[b]
            if s:
                print(f"  {b:12s} n={s['n']:3d} 광란/분 {s['rampage_per_min_med']:5.2f} 무희/분 {s['recklessness_per_min_med']:4.2f} 총시전/분 {s['casts_per_min_med']:5.1f} "
                      f"다운타임 {s['downtime_s_med']:5.1f}s({s['downtime_s_per_min_med']:.1f}s/분) 광역필러 {s['aoe_filler_pct_med']}%")
            else:
                print(f"  {b:12s} (없음)")
        k = d["killtime"]
    ...
    for b in ("rank_1_5", "rank_6_20", "rank_21_100"):
        s = out["overall"][b]
        if s:
            print(f"[{b}] n={s['n']} 광란/분 {s['rampage_per_min_med']} 피갈/분 {s['bloodthirst_per_min_med']} 분강/분 {s['ragingblow_per_min_med']} "
                  f"마일/분 {s['execute_per_min_med']} 무희/분 {s['recklessness_per_min_med']} 총시전/분 {s['casts_per_min_med']} "
                  f"다운타임 {s['downtime_s_med']}s({s['downtime_s_per_min_med']}s/분, 갭 {s['gaps3s_count_med']}회) 광역필러 {s['aoe_filler_pct_med']}%")
    ...
    for hero in HEROES:
        ...
        for b in ("rank_1_5", "rank_6_20", "rank_21_100"):
            ...
            if s:
                print(f"  [{b}] n={s['n']} 광란/분 {s['rampage_per_min_med']} 무희/분 {s['recklessness_per_min_med']} 총시전/분 {s['casts_per_min_med']} "
