"""분노 전사(Fury) 쿨기 사용 패턴 채굴 — 보스별 첫 사용·재사용 간격·홀드·쿨기 정렬.

파싱 패턴은 tmp_mine_mm_cd.py 를 따름
(v2_cache_events.json 1.4GB → raw_decode 증분 스캔, 스크래치패드에 추출 캐시).

사용: python tmp_mine_fury_cd.py explore   → 스펠 ID 관측 (실제 ID 확정용)
      python tmp_mine_fury_cd.py           → 본 분석, data/fury_cd_usage.json 출력
"""
...
OUT_NAME = "fury_cd_usage.json"

# ── 쿨기 후보 (트리 덤프 warr_talent_names.txt 기반, explore 로 확정) ──
# 1719   무모한 희생   (spec row 8)
# 107574 투신          (spec row 11)
# 385059 오딘의 격노   (spec row 10)
# 227847 칼날폭풍      (spec row 11, 투신과 선택 노드)
...
# 버프 창 폴백 길이(버프 이벤트 없을 때 캐스트+N초): 무희 12s, 투신 20s
WIN_FALLBACK = {1719: 12.0, 107574: 20.0}
...
def parse_fight(info, e):
    ...
    # 버프 실측 창 (apply→remove), 없으면 캐스트+폴백
    wins = {}
...
def align_summary(fs):
    """쿨기 정렬: 투신↔무모한 희생 동시(±3s), 칼날폭풍/오딘의 격노가 무희·투신 창 안 비율."""
    out = {}
    reck_n = reck_with_ava = 0
    for f in fs:
        for t in f["sp"].get(1719, []):
            reck_n += 1
            if any(abs(t - a) <= 3 for a in f["sp"].get(107574, [])): reck_with_ava += 1
    out["reck_uses"] = reck_n
    out["reck_with_avatar_3s_pct"] = round(100 * reck_with_ava / reck_n) if reck_n else None

    for sid, label in ((BLADESTORM, "bladestorm"), (385059, "odyns_fury")):
        n = in_reck = in_ava = 0
        for f in fs:
            for t in f["sp"].get(sid, []):
                n += 1
                if any(a <= t <= b for a, b in f["wins"].get(1719, [])): in_reck += 1
                if any(a <= t <= b for a, b in f["wins"].get(107574, [])): in_ava += 1
        out[f"{label}_uses"] = n
        out[f"{label}_in_reck_win_pct"] = round(100 * in_reck / n) if n else None
...
def analyze(wanted, ev, names, heroes):
    ...
    for (eid, bname), lst in sorted(bosses.items()):
        ...
        al = a["align"]
        print(f"  정렬: 무희+투신±3s {al['reck_with_avatar_3s_pct']}%({al['reck_uses']}회) · "
              f"칼폭 in 무희창 {al['bladestorm_in_reck_win_pct']}% in 투신창 {al['bladestorm_in_avatar_win_pct']}%({al['bladestorm_uses']}회) · "
              f"오딘 in 무희창 {al['odyns_fury_in_reck_win_pct']}%({al['odyns_fury_uses']}회)", flush=True)
    ...
    for label in ["all"] + [f"hero_{h}" for h in heroes] + ["rank_1_20", "rank_21_100"]:
        ...
        al = s["align"]
        print(f"    정렬: 무희+투신 {al['reck_with_avatar_3s_pct']}% 칼폭in무희 {al['bladestorm_in_reck_win_pct']}% 오딘in무희 {al['odyns_fury_in_reck_win_pct']}%", flush=True)


# ── 확정 ID (explore 결과로 확정, 2026-07-11) ──────────────────
# 무모한 희생(1719): 296/296판, 판당 ~6.5, 갭 p5=40.5s → 실효 ~40s (분노 제어류 쿨감).
# 투신(107574): 산왕 141판 전원 시전, 학살자 0 — 노드 90415 선택에서 산왕=투신.
# 칼날폭풍: 특성 ID 227847 은 캐스트 0회. 실제 캐스트/버프 ID는 446035 (학살자 155판 전원).
# 오딘의 격노(385059): 385060/385062 와 트리플 이벤트(사용당 1회씩), 갭 p5=20.6s.
# 분쇄의 투척(64382): 296판 중 2캐스트 → 제외. 용사의 창(376079): 0캐스트 → 제외.
BLADESTORM = 446035
TRACKED = {
    1719: "무모한 희생",
    107574: "투신",
...
EXCLUDED = [
    ...
    {"id": 64382, "name": "분쇄의 투척", "reason": "296킬 중 2캐스트 (사실상 미사용)"},
