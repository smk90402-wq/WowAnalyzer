...
장신구 ID 는 하드코딩하지 않는다 — 악마 흑마 상위권 장비(v2 player_fight
...
OUT_JSON = DATA / "demo_trinket_recommendations.json"
...
def load_rankings_by_boss(topn: int) -> dict[int, list[dict]]:
    """오늘자 rankings CSV 에서 악마 흑마 행만 보스별로 (캐시-명단 정합)."""
...
def main() -> None:
    ...
    output = {
        "_meta": {
            ...
            "notes": [
                ...
                "장신구 목록은 하드코딩 없이 악마 흑마 상위권 장비에 실제로 나온 것만 집계합니다.",
    ...
    try:
        ...
        record(
            action="analyze_demo_trinket_recommendations",
