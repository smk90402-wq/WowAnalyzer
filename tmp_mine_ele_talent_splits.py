# -*- coding: utf-8 -*-
"""정기 주술사(Elemental) 특성 노드 보스별 분기 실측 — rank 1~20 vs 21~100.

패턴은 tmp_mine_fury_talent_splits.py 를 그대로 따름.

입력(캐시만, API 호출 없음):
  data/rankings_zone46_mythic_dps_top100.csv  (Shaman/Elemental 900행)
  data/v2_cache_player_fight.json             (nodes/talents/talent_points)
  data/talent_trees.json                      ('Shaman/Elemental' — 노드/선택지 한글명)

영웅특성: 정기는 폭풍인도자(Stormbringer) vs 선견자(Farseer).
  (talent_trees.json 의 hero 딕셔너리에는 '토템술사'도 들어있으나 이는 Blizzard
   API가 주술사 클래스트리 전체를 반환하는 특성 — 실제 정기는 폭풍인도자/선견자만
   선택 가능. 실측으로도 토템술사 채택 0명이면 그대로 확인.)

이름 원칙: talent_trees.json 의 공식 한글명 그대로 사용.
선택(CHOICE) 노드의 플레이어측 entry_id ↔ 옵션 매핑:
  노드 안에서 entry_id 오름차순 = 옵션 talent_id 오름차순 대응.
  주술사(신규 클래스)에서 재검증함(2026-07-11): 단일옵션 노드의 (entry_id, talent_id)
  실측쌍에서 entry 대역별 오프셋(117xxx→+5012, 127xxx→+4809, 135xxx→+4755/4756,
  136xxx→+4763~4773)이 일정. 101xxx 구세대 대역만 노드별 미세 변동(+4950~+5012).
  양쪽 옵션이 모두 관측된 CHOICE 노드 15건 중 14건이 대역 오프셋과 정확 부합,
  나머지 1건(80998)은 101xxx 대역 미세 변동 범위 안(+4945)이고 교차 pairing 은
  오프셋 3만+ 로 불가능 → 오름차순이 유일하게 정합.
출력: data/ele_talent_splits.json
"""
...
OUT = DATA / "ele_talent_splits.json"

SPEC_CLASS, SPEC_NAME = "Shaman", "Elemental"
TREE_KEY = "Shaman/Elemental"
CLASS_TREE_LABEL = "직업(주술사 공용)"
SPEC_TREE_LABEL = "정기 전문화"
HERO_A, HERO_B = "폭풍인도자", "선견자"     # 실제 선택 가능한 두 영웅특성
HERO_IMPOSSIBLE = "토템술사"                # API 반환용 — 정기 선택 불가
# 99845: talent_trees.json 상 options=[] 인 CHOICE 마커 노드. 실측 entry 카운트
# (123376/123377)가 영웅트리 인원수와 일치하는지 실행 시 확인 — 영웅트리 선택 마커 → 제외.
DEAD_UNMAPPED_NODES = {99845}
...
def load_tree():
    ...
    take(tr["class"], CLASS_TREE_LABEL)
    take(tr["spec"], SPEC_TREE_LABEL)
    hero_sets = {}
    for tname, h in tr["hero"].items():
...
def load_sample():
    ...
    out = []
    for r in rows:
        if r["class"] != SPEC_CLASS or r["spec"].replace(" ", "") != SPEC_NAME:
            continue
...
def main() -> None:
    ...
    # ---------- 2b) 영웅특성별 분리 채택률 ----------
    hero_groups = {h: [s for s in S if s["hero"] == h] for h in (HERO_A, HERO_B)}
    hero_node_rate = {}
    ...
    # (주술사 표본 단일옵션 노드의 entry 대역별 오프셋 실측으로 재검증 — 파일 상단 주석 참조)
    choice_nodes = {}
    for nid, tc in pairs.items():
        ...
        choice_nodes[str(nid)] = {
            ...
            "picked_by": total,
            "mapping_basis": "entry_id 오름차순=talent_id 오름차순 (주술사 표본 단일옵션 노드 entry 대역별 오프셋 실측 재검증, CHOICE 15건 중 14건 정확 부합·1건 대역내 미세변동)",
            "options": opts,
        }
    ...
    # 영웅특성 고정 후에도 남는 보스별 진폭 → 진짜 기믹 스왑
    for u in util_swaps:
        nid = u["node"]
        ctrl = {}
        for hero_label in (HERO_A, HERO_B):
            per = {}
            for bid in BOSS_ORDER:
    ...
    out = {
        "_meta": {
            "source": "rankings_zone46_mythic_dps_top100.csv(2026-07-10) + v2_cache_player_fight.json + talent_trees.json — API 호출 없음",
            "spec": f"{SPEC_CLASS}|{SPEC_NAME}", "sample_n": n, "top_band": TOP_BAND,
            "naming_rule": f"노드/선택지 한글명은 talent_trees.json('{TREE_KEY}') 공식 데이터 그대로.",
            "hero_note": f"정기는 {HERO_A} vs {HERO_B}. '{HERO_IMPOSSIBLE}'은 API가 주술사 클래스트리 전체를 반환해 생긴 항목 — 실제 정기 선택 불가.",
        },
        "hero_tree": {
    ...
    ca = hero_total.get(HERO_A, 0)
    cb = hero_total.get(HERO_B, 0)
    ci = hero_total.get(HERO_IMPOSSIBLE, 0)
    majority = HERO_A if ca >= cb else HERO_B
    out["hero_tree"]["comment"] = (
        f"{n}명 중 {HERO_A} {ca}, {HERO_B} {cb}"
        + (f", {HERO_IMPOSSIBLE} {ci}" if ci else "")
        + f". 다수파: {majority} ({max(ca, cb) / n:.0%})."
        + (f" {HERO_IMPOSSIBLE}은 API가 주술사 클래스트리 전체를 반환해 생긴 항목 — 실제 정기 선택 불가, 채택 0명으로 확인." if ci == 0 else "")
    )
    ...
    for bid in BOSS_ORDER:
        b = BOSS_KR[bid]
        c = hero_by_boss[b]
        top = Counter(s["hero"] for s in S if s["boss_id"] == bid and s["rank"] <= TOP_BAND)
        print(f"  {b}: 전체 {dict(c)} (합 {sum(c.values())}) | top20 {dict(top)}")
    print("\n=== 정점 특성 ===")
