# -*- coding: utf-8 -*-
"""무기 전사(Arms) 특성 노드 보스별 분기 실측 — rank 1~20 vs 21~100.

패턴은 tmp_mine_mm_talent_splits.py 를 그대로 따름.

입력(캐시만, API 호출 없음):
  data/rankings_zone46_mythic_dps_top100.csv  (Warrior/Arms 900행)
  data/v2_cache_player_fight.json             (nodes/talents/talent_points)
  data/talent_trees.json                      ('Warrior/Arms' — 노드/선택지 한글명)

영웅특성: 무기는 학살자(Slayer) vs 거신(Colossus).
  (talent_trees.json 의 hero 딕셔너리에는 '산왕'도 들어있으나 이는 Blizzard API가
   전사 클래스트리 전체를 반환하는 특성 — 실제 무기는 학살자/거신만 선택 가능.
   실측으로도 산왕 채택 0명이면 그대로 확인.)

이름 원칙: talent_trees.json 의 공식 한글명 그대로 사용.
선택(CHOICE) 노드의 플레이어측 entry_id ↔ 옵션 매핑:
  노드 안에서 entry_id 오름차순 = 옵션 talent_id 오름차순 대응.
  전사에서 재검증함: 단일옵션 노드 89건의 (entry_id, talent_id) 실측쌍에서 entry
...
출력: data/arms_talent_splits.json
"""
...
OUT = DATA / "arms_talent_splits.json"

SPEC_CLASS, SPEC_NAME = "Warrior", "Arms"
TREE_KEY = "Warrior/Arms"
CLASS_TREE_LABEL = "직업(전사 공용)"
SPEC_TREE_LABEL = "무기 전문화"
HERO_A, HERO_B = "학살자", "거신"          # 실제 선택 가능한 두 영웅특성
HERO_IMPOSSIBLE = "산왕"                    # API 반환용 — 무기 선택 불가
# 99853: talent_trees.json 상 options=[] 인 CHOICE 마커 노드. 실측 entry 카운트
# (123390→학살자 인원수, 123393→거신 인원수와 정확 일치) = 영웅트리 선택 마커 → 제외.
DEAD_UNMAPPED_NODES = {99853}
...
def main() -> None:
    ...
    # (전사 단일옵션 노드 89건의 entry 대역별 오프셋 실측으로 검증 — 파일 상단 주석 참조)
    choice_nodes = {}
    for nid, tc in pairs.items():
        ...
        choice_nodes[str(nid)] = {
            ...
            "picked_by": total,
            "mapping_basis": "entry_id 오름차순=talent_id 오름차순 (전사 단일옵션 노드 89건의 entry 대역별 오프셋 실측 검증)",
            "options": opts,
        }
    ...
    out = {
        "_meta": {
            ...
            "hero_note": f"무기는 {HERO_A} vs {HERO_B}. '{HERO_IMPOSSIBLE}'은 API가 전사 클래스트리 전체를 반환해 생긴 항목 — 실제 무기 선택 불가.",
        },
        "hero_tree": {
    ...
    out["hero_tree"]["comment"] = (
        ...
        + (f" {HERO_IMPOSSIBLE}은 API가 전사 클래스트리 전체를 반환해 생긴 항목 — 실제 무기 선택 불가, 채택 0명으로 확인." if ci == 0 else "")
    )
