"""사격 사냥꾼 보스별 장신구 추천 분석.

WCL 신화 사격 랭킹(최신 파티션)을 받아 V2 combatantInfo 로 장신구를 붙이고,
"평균 딜"보다 "99 파스 확률을 올리는 세팅" 관점의 JSON 리포트를 만든다.

Frost 버전(analyze_frost_trinket_recommendations.py)과 마찬가지로 장신구 ID 를
하드코딩하지 않는다 — 사격 상위권 장비(v2 player_fight 캐시)에 실제로 등장한
장신구만 집계. 이름은 data/item_db.json 의 한글명 → 착용자 gear 의 영문명 →
"item {id}" 순. 영웅특성(어둠 순찰자/파수꾼) 채택 인원도 같이 기록한다.
"""
...
OUT_JSON = DATA / "mm_trinket_recommendations.json"
TALENT_TREES = DATA / "talent_trees.json"
SPEC_KEY = "Hunter|Marksmanship"
ZONE_ID = 46
DIFFICULTY = 5
...
def load_hero_nodes() -> dict[str, set[int]]:
    """talent_trees.json 에 Hunter/Marksmanship 영웅트리가 있으면 {이름: 노드셋} 반환.
    ...
    try:
        ...
        return {}
    hero = (tt.get("Hunter/Marksmanship") or {}).get("hero") or {}
...
def main() -> None:
    ...
    output = {
        "_meta": {
            ...
            "notes": [
                ...
                "장신구 목록은 하드코딩 없이 사격 상위권 장비에 실제로 나온 것만 집계합니다.",
    ...
    try:
        ...
        record(
            action="analyze_mm_trinket_recommendations",
