# -*- coding: utf-8 -*-
"""생존 사냥꾼(Survival) 특성 노드 보스별 분기 실측 — rank 1~20 vs 21~100.

패턴은 tmp_mine_fury_talent_splits.py 를 그대로 따름.

입력(캐시만, API 호출 없음):
  data/rankings_zone46_mythic_dps_top100.csv  (Hunter/Survival 900행)
  data/v2_cache_player_fight.json             (nodes/talents/talent_points)
  data/talent_trees.json                      ('Hunter/Survival' — 노드/선택지 한글명)

영웅특성: 생존은 무리의 지도자(Pack Leader) vs 파수꾼(Sentinel).
  (talent_trees.json 의 hero 딕셔너리에는 '어둠 순찰자'도 들어있으나 이는 Blizzard
   API가 사냥꾼 클래스트리 전체를 반환하는 특성 — 실제 생존은 무리의 지도자/파수꾼만
   선택 가능. 실측으로도 어둠 순찰자 채택 0명이면 그대로 확인.)

이름 원칙: talent_trees.json 의 공식 한글명 그대로 사용.
선택(CHOICE) 노드의 플레이어측 entry_id ↔ 옵션 매핑:
  노드 안에서 entry_id 오름차순 = 옵션 talent_id 오름차순 대응.
  생존 표본에서 재검증함(2026-07-11): 단일옵션 노드의 (entry_id, talent_id) 실측쌍에서
  entry 대역별 오프셋(100xxx→+5002, 117xxx→+5012, 126xxx→+4826, 128xxx→+4806,
  135xxx→+4755~4757, 136xxx→+4755/4772, 137xxx→+4763~4765)이 대역 내 일정하고,
  양쪽 옵션이 모두 관측된 CHOICE 노드 5건 전부 오름차순 zip 이 이 대역 오프셋과 부합
  (교차 pairing 은 오프셋 수만 단위로 불가능).
출력: data/sv_talent_splits.json
"""
...
OUT = DATA / "sv_talent_splits.json"

SPEC_CLASS, SPEC_NAME = "Hunter", "Survival"
TREE_KEY = "Hunter/Survival"
CLASS_TREE_LABEL = "직업(사냥꾼 공용)"
SPEC_TREE_LABEL = "생존 전문화"
HERO_A, HERO_B = "무리의 지도자", "파수꾼"  # 실제 선택 가능한 두 영웅특성
HERO_IMPOSSIBLE = "어둠 순찰자"             # API 반환용 — 생존 선택 불가
# 99833: talent_trees.json 상 options=[] 인 마커 노드. 실측 entry 는 123350 단일값
# (표본 전원 동일) — 이름 매핑 불가한 내부 마커 → 제외.
DEAD_UNMAPPED_NODES = {99833}
...
def main() -> None:
    ...
    # (생존 표본 단일옵션 노드의 entry 대역별 오프셋 실측으로 재검증 — 파일 상단 주석 참조)
    choice_nodes = {}
    for nid, tc in pairs.items():
        ...
        choice_nodes[str(nid)] = {
            ...
            "picked_by": total,
            "mapping_basis": "entry_id 오름차순=talent_id 오름차순 (생존 표본 단일옵션 노드 entry 대역별 오프셋 실측 재검증, CHOICE 5건 전부 정합)",
            "options": opts,
        }
    ...
    out = {
        "_meta": {
            ...
            "hero_note": f"생존은 {HERO_A} vs {HERO_B}. '{HERO_IMPOSSIBLE}'은 API가 사냥꾼 클래스트리 전체를 반환해 생긴 항목 — 실제 생존 선택 불가.",
        },
        "hero_tree": {
    ...
    out["hero_tree"]["comment"] = (
        ...
        + (f" {HERO_IMPOSSIBLE}은 API가 사냥꾼 클래스트리 전체를 반환해 생긴 항목 — 실제 생존 선택 불가, 채택 0명으로 확인." if ci == 0 else "")
    )
