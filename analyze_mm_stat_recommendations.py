"""사격 사냥꾼 보스별 스탯 우선순위 생성기.

신화 top100 랭킹 스냅샷 + player_fight 스탯 캐시를 읽어
data/mm_stat_recommendations.json 을 만든다 (스탯 탭 소비용).
...
MM 전용 개념은 없고, 1~20등 개별 스탯 + 21~100등 평균과
스탯 비중, 우선순위 추천만 낸다. 영웅특성(어둠 순찰자/파수꾼) 채택 인원도 같이 기록한다.
"""
...
OUT_JSON = DATA_DIR / "mm_stat_recommendations.json"
...
def main() -> None:
    ...
    try:
        ...
        record(
            action="analyze_mm_stat_recommendations",
