"""고통 흑마법사 보스별 스탯 우선순위 생성기.

신화 top100 랭킹 스냅샷 + player_fight 스탯 캐시를 읽어
data/aff_stat_recommendations.json 을 만든다 (스탯 탭 소비용).
...
영웅특성(고통환기/영혼 수확자)은 talent_trees.json 노드 교집합으로 판별.
"""
...
OUT_JSON = DATA_DIR / "aff_stat_recommendations.json"
...
def main() -> None:
    ...
    try:
        ...
        record(
            action="analyze_aff_stat_recommendations",
