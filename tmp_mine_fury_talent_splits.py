# -*- coding: utf-8 -*-
"""분노 전사(Fury) 특성 노드 보스별 분기 실측 — rank 1~20 vs 21~100.

패턴은 tmp_mine_mm_talent_splits.py 를 그대로 따름.

입력(캐시만, API 호출 없음):
  data/rankings_zone46_mythic_dps_top100.csv  (Warrior/Fury 900행)
  data/v2_cache_player_fight.json             (nodes/talents/talent_points)
  data/talent_trees.json                      ('Warrior/Fury' — 노드/선택지 한글명)

영웅특성: 분노는 학살자(Slayer) vs 산왕(Mountain Thane).
  (talent_trees.json 의 hero 딕셔너리에는 '거신'도 들어있으나 이는 Blizzard API가
   전사 클래스트리 전체를 반환하는 특성 — 실제 분노는 학살자/산왕만 선택 가능.
   실측으로도 거신 채택 0명이면 그대로 확인.)

이름 원칙: talent_trees.json 의 공식 한글명 그대로 사용.
선택(CHOICE) 노드의 플레이어측 entry_id ↔ 옵션 매핑:
  노드 안에서 entry_id 오름차순 = 옵션 talent_id 오름차순 대응.
  전사에서 재검증함: 단일옵션 노드 90건의 (entry_id, talent_id) 실측쌍에서 entry
  대역별 오프셋(112xxx→+5005, 117xxx→+5012, 118xxx→+4900, 123xxx→+4838,
  134xxx→+477x)이 일정하고, 오름차순 zip 결과가 이 대역 오프셋과 전부 부합.
  (예외 108705 위협의 외침/날카로운 고함은 오프셋 4773/4772로 대역 내 미세 변동,
   교차 pairing 은 오프셋 +7145/+2400 로 불가능 → 오름차순이 유일하게 정합.)
출력: data/fury_talent_splits.json
"""
...
OUT = DATA / "fury_talent_splits.json"

SPEC_CLASS, SPEC_NAME = "Warrior", "Fury"
TREE_KEY = "Warrior/Fury"
CLASS_TREE_LABEL = "직업(전사 공용)"
SPEC_TREE_LABEL = "분노 전문화"
HERO_A, HERO_B = "학살자", "산왕"          # 실제 선택 가능한 두 영웅특성
HERO_IMPOSSIBLE = "거신"                    # API 반환용 — 분노 선택 불가
# 99852: talent_trees.json 상 options=[] 인 CHOICE 마커 노드. 실측 entry 카운트
# (123389→학살자 인원수, 123392→산왕 인원수와 정확 일치) = 영웅트리 선택 마커 → 제외.
DEAD_UNMAPPED_NODES = {99852}
...
def main() -> None:
    ...
    # (전사 단일옵션 노드 90건의 entry 대역별 오프셋 실측으로 검증 — 파일 상단 주석 참조)
    choice_nodes = {}
    for nid, tc in pairs.items():
        ...
        choice_nodes[str(nid)] = {
            ...
            "picked_by": total,
            "mapping_basis": "entry_id 오름차순=talent_id 오름차순 (전사 단일옵션 노드 90건의 entry 대역별 오프셋 실측 검증)",
            "options": opts,
        }
    ...
    out = {
        "_meta": {
            ...
            "hero_note": f"분노는 {HERO_A} vs {HERO_B}. '{HERO_IMPOSSIBLE}'은 API가 전사 클래스트리 전체를 반환해 생긴 항목 — 실제 분노 선택 불가.",
        },
        "hero_tree": {
    ...
    out["hero_tree"]["comment"] = (
        ...
        + (f" {HERO_IMPOSSIBLE}은 API가 전사 클래스트리 전체를 반환해 생긴 항목 — 실제 분노 선택 불가, 채택 0명으로 확인." if ci == 0 else "")
    )
