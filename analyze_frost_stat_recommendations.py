"""냉기 법사 보스별 스탯 우선순위 생성기.

신화 top100 랭킹 스냅샷 + player_fight 스탯 캐시를 읽어
data/frost_stat_recommendations.json 을 만든다 (스탯 탭 소비용).

방법은 BM 버전(analyze_bm_stat_recommendations.py)과 동일:
DPS 에서 템렙·전투시간 영향을 뺀 값과 스탯 수치의 상관을 보고,
보스별로 "상위권이 실제로 챙기는 스탯 순서" 를 뽑는다.
BM 전용 개념(꽁지깃 등)은 없고, 1~20등 개별 스탯 + 21~100등 평균과
스탯 비중, 우선순위 추천만 낸다.
"""
...
OUT_JSON = DATA_DIR / "frost_stat_recommendations.json"
SPEC_KEY = "Mage|Frost"
CLASS_NAME = "Mage"
SPEC_NAME = "Frost"
...
def main() -> None:
    ...
    try:
        ...
        record(
            action="analyze_frost_stat_recommendations",
