...
def spell_map() -> Response:
    ...
    if _SPELL_MAP_CACHE is None:
        ...
        override = {
            ...
            "얼음 화살": 116, "진눈깨비": 44614, "얼음창": 30455,
            "얼어붙은 구슬": 84714, "혹한의 쐐기": 199786, "서리 광선": 205021,
            "두뇌 빙결": 190446, "서리의 손가락": 44544, "열기 동공": 1247730,
            "눈보라": 190356, "쇄편폭풍": 1247908, "얼음불꽃 화살": 431044,
            "얼음 보호막": 11426, "얼음 방패": 45438, "오색 방벽": 235450,
            # 전사 (2026-07-11 실측 확정 ID — 채굴 노트/트리 검증. 2글자 이름이 많아
            # 자동매핑 최소길이(4)에 걸리므로 전부 override 로)
            "광란": 184367, "피의 갈증": 23881, "피범벅": 335096,
            "분노의 강타": 85288, "분쇄의 타격": 335097, "마무리 일격": 5308,
            "오딘의 격노": 385059, "무모한 희생": 1719, "투신": 107574,
            "칼날폭풍": 446035, "소용돌이": 190411, "천둥벼락": 6343,
            "우레 작렬": 435222, "급살": 52437, "격노": 184362,
            "필사의 일격": 12294, "제압": 7384, "거인의 강타": 167105,
            "영웅의 일격": 1269383, "전쟁의 지배자": 1269391, "격돌": 1464,
            "회전베기": 845, "분쇄": 772, "쇄파": 436358, "쇠날발톱": 228920,
            "휩쓸기 일격": 260708, "대학살": 206315, "분노 제어": 152278,
            "집행자의 정밀함": 386633, "거인의 힘": 440989, "돌진": 100,
            "소용돌이 연마": 12950, "폭발적인 힘": 437118,
        }
...
# ★사용자 교정(핵심)★: 정공 로스터의 직업분포(킬로그 TO)가 아니라, "막공/신규정공이 자리를 못 채워
#   구인 시장에 올리는 직업"이 환영지표다. 정해진 공대 구성보다 '구인 시장에서 중요한 직업'이 뭐냐.
# 방법 (data/pug_demand_inven.json, 2026-06-15):
#  · 인벤 board 2972 공격대_구인 본문 138개 고유 캠페인(작성자+제목 중복제거) 정독 → DPS 스펙 수요 집계
#    (제목은 "딜러 구인(세부참조)"뿐, 실제 스펙은 본문 약어 "딜4(흑,암,정,냥)" → 파서로 추출)
#  · 구인 수요(breadth): 몇 공대가 찾나 = 자리 폭. 사냥꾼30·증강24·정술21·죽기20·흑마19 (top)
#  · 구인 난이도(수요÷공급): 희소한데 수요있나 = 콕집어 모심. 황폐33·증강17·사냥꾼15·정술14 / 징벌1(과잉)
#  · 수요는 클래스 단위(냥꾼=헌터 전체) → 클래스 내 메타 스펙으로 라우팅, 비메타 스펙은 하향
# ★구버전 교정★: 냉법 5→4(법사 구인 9 중위 + 공급과잉 → 신규진입 경쟁적, 로스터 1.71은 정공 누적 착시),
#   황폐 1→4(구인 7 + 공급 최소 211 → '구인 어려운 직업'=콕집어 환영. 로스터 TO 0.03이 놓친 신호),
#   포식 5→3(구인 5뿐). 징벌 2 유지(구인 2 + 공급 2097 → 수요/공급 1.0 꼴찌). 점수 미반영 참고 컬럼.
PUG_WELCOME: dict[tuple[str, str], tuple[int, str]] = {
    # 5 = 구인 시장 최상위 수요 (메타 스펙)
    ("Hunter", "Marksmanship"): (5, "구인 수요 1위 — '냥꾼' 30/138 공대(22%)가 찾음. 사격이 선호 스펙. 원딜·러스트·유연성"),
    ("Hunter", "Beast Mastery"): (5, "'냥꾼' 구인 30 공유 — 사격 선호지만 야수도 무방(사용자 본캐). 원딜·러스트"),
    ("Evoker", "Augmentation"): (5, "구인 27(2위, '용딜'=증강 포함) + 기원사 딜자리 독점 — 버퍼 1자리 고정 수요"),
    ("Shaman", "Elemental"): (5, "구인 21(3위) — 술사 유일 현실 딜스펙, 러스트. 복원 겸업으로 스왑힐 자리도 충족"),
    ("Death Knight", "Unholy"): (5, "죽기 구인 18(상위) — '죽딜'은 부정/냉죽 영특 구분 안 함(부정이 주력). 근딜 유일 콕집힘, 브레즈·대마법지대"),
    ("Warlock", "Demonology"): (5, "흑마 구인 19(5위) — 영웅 1주차 딜 1위, 브레즈·관문, 흑마 주력"),
    # 4 = 환영
    ("Warlock", "Destruction"): (4, "흑마 구인 19 공유(2nd 딜스펙) — 흑마 클래스 수요가 받쳐줌"),
    ("Rogue", "Subtlety"): (4, "도적 구인 8 + 수요/공급 12.6 — 도적 유일 메타 스펙, 희소(공급 637)해 콕집힘"),
    ("Mage", "Frost"): (4, "법사 구인 9(중위) — 메타 법사지만 공급 과잉(2331, 수요/공급 3.9)이라 신규 진입은 경쟁적 (정공 1.71은 누적 착시)"),
    ("Death Knight", "Frost"): (4, "★'죽딜 구인'이 부정/냉죽 영특 구분 안 함(냉죽 단일딜 양호) — 죽기 수요 18을 부정과 공유. 혈죽은 탱(별개)"),
    ("Priest", "Shadow"): (4, "암사 구인 6 + 마력주입 + ★수양 겸업으로 스왑힐 12자리(수암)도 충족 — 딜+힐 양쪽 자리"),
    ("Druid", "Balance"): (4, "조드 구인 6 + 브레즈·야생의 징표 + ★회복 겸업으로 스왑힐(회조)도 충족 — 딜+힐 양쪽 자리"),
    # 3 = 무난
    ("Demon Hunter", "Devourer"): (3, "포식 구인 5뿐(로스터 1.22는 과대) — 25야드 유사원거리지만 구인 시장 수요는 중위"),
    # 2 = 찬밥
    ("Demon Hunter", "Havoc"): (2, "악딜 구인 3 — 악사 자리를 포식이 우선, 혼돈의 낙인은 포식이 대체"),
    ("Monk", "Windwalker"): (2, "풍운 구인 2 + 수요/공급 1.9 — 수도사 자리는 운무·양조에 쏠림"),
    ("Warrior", "Arms"): (2, "전사 구인 2뿐 — 전투의 외침 유틸 외 구인 시장 수요 낮음"),
    ("Warrior", "Fury"): (2, "전사 구인 2뿐 — 무기와 동급, 구인 시장에서 잘 안 찾음"),
    ("Paladin", "Retribution"): (2, "★구인 2 + 공급 2097 → 수요/공급 1.0 꼴찌 = 과잉공급. 신규 자리 가장 어려움(본인 증언 일치)"),
    ("Warlock", "Affliction"): (2, "흑마 클래스 수요는 강하나 구인은 악마·파괴에 쏠림 — 고통 본캐 자리 적음"),
    ("Evoker", "Devastation"): (2, "★용딜=증강 교정: '용딜 구인'은 황폐 아닌 증강. 황폐 명시 수요 4뿐 + 공급 211 멸종급 — 기원사 딜자리는 증강 독점"),
    # 1 = 구인 시장 수요 거의 없음
    ("Hunter", "Survival"): (1, "근딜 사냥꾼 — 사냥꾼 구인 30이 전부 원딜로 감(근딜 12 vs 원딜 34). 생존 콕집힘 0"),
    ("Mage", "Fire"): (1, "화염 구인 0 — 법사 구인은 냉법으로, 화염 본캐 자리 없음"),
    ("Mage", "Arcane"): (1, "비전 구인 0 — 법사 비메타, 신규 자리 없음"),
    ("Rogue", "Outlaw"): (1, "무법 구인 0 — 도적 구인 8이 전부 잠행으로"),
    ("Rogue", "Assassination"): (1, "암살 구인 0 — 도적 비메타, 잠행 스왑이 입장권"),
    ("Shaman", "Enhancement"): (1, "고양 구인 0 — 술사 구인 21이 전부 정술로, 러스트 가치도 정술에 흡수"),
    ("Druid", "Feral"): (1, "야성 구인 0 — 드루 구인은 조드로, 근딜 야성 자리 없음"),
}
